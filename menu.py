...
def menu(minimum: int, maximum: int, lang: str):
    for d in scrape_areas(minimum, maximum):
        with keep.presenting():
            ...
            response["area_id"] = d["area_id"]
            response["country_name"] = d["country_name"]
            response["area_name"] = d["area_name"]
...
Thread(name="Talabat Menu Scraper EN 10", daemon=False, args=(22000, 23000, "en"), target=menu).start()
